(function(){
	...
	treeView.buildTree = function(obj){
	...
	treeView.collapse = function(event){
	...
	treeView.check_highline = function(event){
		var event = event || window.event,
		    target = event.target;
        if (target.tagName !== "A"){
            return;
        }
		if($('#highline')[0] !== undefined && $('#highline')[0] !== target){
			var el = $('#highline')[0];
			el.id = el.id.replace(/\bhighline\b/,'');		
		}
	}

	treeView.highline = function(event){ 
		treeView.check_highline(event); 
        var event 				  = event || window.event,
            target 				  = event.target,
		    additional_form       = $('#add-node')[0],
	...
	treeView.add_node = function(event){ 
		var el 		= $('#highline')[0],
	...
	treeView.context_menu = function(event){
		target = event.target || event.srcElement;
        event.preventDefault();
    	...
    	addEvent(create_link, 'click', treeView.modal);
	...
	treeView.context_menu.hide = function(){
		var context_menu 	= document.getElementById('context-menu'),
			rename_link 	= document.getElementById('rename-link'),
        	delete_link 	= document.getElementById('delete-link'),
        	create_link 	= document.getElementById('create-link');
    ...
    treeView.context_menu.rename_click = function(e){
    ...
    treeView.context_menu.rename_blur = function(e){
        if (e.keyCode ==13){
            var data 	= this.value,
                a       = document.createElement('a');
            a.innerHTML = data;
    ...
    treeView.context_menu.delete_click = function(){
        var tree    = treeView.localStorage.read('tree'),
            arr     = [],
            additional_form = $('#add-node')[0];
    ...
    treeView.context_menu.create_click = function(){
        var ul 		= document.createElement('ul'),
        	li 		= document.createElement('li'),
			span1 	= document.createElement('span'),
			span2 	= document.createElement('span'),
        ...
        if (data !== ""){
            var a = document.createElement('a'),
                tree = treeView.localStorage.read('tree');
    ...
    treeView.modal = function(){
        ...
        text.focus();
    	addEvent(close, 'click', treeView.modal.hide);
    	addEvent(f, 'submit', treeView.context_menu.create_click);
    }

   	treeView.modal.hide = function(){
        ...
        text.value = '';
        removeEvent(close, 'click', treeView.modal.hide);
        removeEvent(f, 'submit', treeView.modal.hide);
